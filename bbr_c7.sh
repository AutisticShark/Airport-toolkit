...
export PATH
cat << "EOF"
BBR configuration script for RHEL/CentOS 7 x86_64
Author: M1Screw
Github: https://github.com/M1Screw/Airport-toolkit
Usage:
./bbr_c7.sh mainline --> Install mainline kernel and enable BBR
./bbr_c7.sh longterm --> Install longterm kernel and enable BBR
./bbr_c7.sh bbr --> Enable BBR
./bbr_c7.sh status --> Check BBR and kernel status
./bbr_c7.sh update --> Update the mainline/longterm kernel                                
EOF
